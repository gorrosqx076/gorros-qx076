...
// ==============================================
// --- COMPRA DIRECTA (BOTÓN COMPRAR AHORA)
// ==============================================
...
// ========================================================
// --- MENÚ DINÁMICO (MODO ELEVADOR / SCROLL DIRECTO) ---
// ========================================================
...
function renderMenuCategoriasFrontend() {
    ...
    // El botón de "Ver todo" lo podemos mandar hasta arriba del catálogo
    let html = `<a href="#" onclick="bajarAColeccion('inicio-catalogo'); return false;" style="font-weight:bold; color:var(--vino);">Ver Todo el Catálogo</a>
               <hr style="margin: 5px 0; border: 0; border-top: 1px solid #eee;">`;

    for(const [id, nombre] of Object.entries(originalesBase)) {
        if(!categoriasBorradasCache[id]) { 
            // Fíjate cómo ahora llama a "bajarAColeccion" y le pasa el ID
            html += `<a href="#" onclick="bajarAColeccion('${id}'); return false;">${nombre}</a>`; 
        }
    }
    for(const [id, cat] of Object.entries(categoriasExtraCache)) {
        html += `<a href="#" onclick="bajarAColeccion('${id}'); return false;">${cat.nombre}</a>`;
    }
    
    menu.innerHTML = html;
}

// 🚀 FUNCIÓN QUE TE BAJA EXACTAMENTE AL TÍTULO DE LA COLECCIÓN
function bajarAColeccion(idCategoria) {
    // Magia aquí: Buscamos primero el ID exacto, y si no existe, probamos agregándole el prefijo "cat-"
    const seccion = document.getElementById(idCategoria) || document.getElementById('cat-' + idCategoria);
    
    if(seccion) {
        // Le restamos unos 80px (o lo que mida tu menú de arriba) para que el título no quede tapado
        const offset = 80; 
        const topPos = seccion.getBoundingClientRect().top + window.scrollY - offset;
        ...
        console.warn("⚠️ No se encontró en la página ninguna sección con el ID: " + idCategoria + " ni cat-" + idCategoria);
    }
}
// ========================================================
...
let estabaEnMantenimiento = false;

db.ref('Configuracion/Mantenimiento/activo').on('value', (snap) => {
    const enMantenimiento = snap.val();
    
    if (enMantenimiento) {
        estabaEnMantenimiento = true;
        // Inyectamos la pantalla rosa que tapa absolutamente todo
       // Inyectamos la pantalla rosa que tapa absolutamente todo
        document.body.innerHTML = `
            <div style="position:fixed; top:0; left:0; width:100vw; height:100vh; background: #E96B85; display:flex; flex-direction:column; justify-content:center; align-items:center; z-index:9999999; color:white; font-family:'Outfit', sans-serif; text-align:center; padding: 20px; box-sizing: border-box;">
                <img src="https://i.postimg.cc/1XjdJtsP/Gemini-Generated-Image-52h3mj52h3mj52h3-(1).png" alt="Logo" style="height:140px; filter:invert(1) drop-shadow(0px 4px 10px rgba(0,0,0,0.3)); margin-bottom: 25px; animation: pulse 2s infinite;">
                <h1 style="font-family:'Playfair Display', serif; font-size:3rem; margin:0 0 15px 0; text-shadow: 0 4px 10px rgba(0,0,0,0.2);">Pausa por Mantenimiento</h1>
                <p style="font-size:1.1rem; max-width:500px; line-height:1.6; font-weight: 300;">Estamos optimizando nuestra plataforma para ofrecerte una experiencia de compra excepcional. Agradecemos tu paciencia.</p>
                <div style="margin-top: 30px; padding: 12px 25px; background: rgba(255,255,255,0.15); border-radius: 30px; border: 1px solid rgba(255,255,255,0.3); font-weight: 600; letter-spacing: 1px; text-transform: uppercase; font-size: 0.9rem;">
                    Regresamos en breve
                </div>
            </div>
            <style>
        ...
        `;
    } else if (estabaEnMantenimiento) {
        // Si lo apagaron y la página estaba en pantalla rosa, la recargamos para que todo vuelva a cargar normal
